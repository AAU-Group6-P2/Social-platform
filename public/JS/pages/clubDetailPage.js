...
async function init() {
   ...
   if (club.image) {
    ...
    if (clubEvents.length === 0) {
        ...
        for (const event of clubEvents) {
            ...
            if (isOwner) {
                ...
                actions.appendChild(joinCount);
                actions.insertAdjacentHTML("beforeend", `<button class="button edit-event-button blue-btn">Edit</button>`);
            } else if (userRole === "student") {
                const joinButton = clone.querySelector(".join-event-button");
                joinButton.classList.remove("hidden");
                await setupEventJoinButton(joinButton, event.id);
            }
    ...
    if(userRole === "student"){
        ...
        await setupClubJoinButton(joinbtn, clubId);
    }
